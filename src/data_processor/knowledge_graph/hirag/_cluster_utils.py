...
def GMM_cluster(embeddings: np.ndarray, threshold: float, random_state: int = 0):
    ...
    n_clusters = max(n_clusters, len(embeddings) // target_cluster_size)
    gm = GaussianMixture(
            ...
            for label in unique_clusters:
                ...
                total_length = sum(
                    [
                        ...
                        + len(tokenizer.encode(str(node["entity_name"])))
                ...
                summarize_prompt = PROMPTS["summary_clusters_new"]
                summarize_result = ""
                for i in range(int(len(entity_description_list) / 50) + 1):
                    if i != len(entity_description_list) / 50:
                        entity_description_l = ",".join(
                            entity_description_list[i * 50 : i * 50 + 50]
                        )
                    else:
                        entity_description_l = ",".join(
                            entity_description_list[i * 50 :]
                        )
                    context_base_summarize = dict(
                        ...
                        entity_description_list=entity_description_l,
                    )
                ...
                # logging.info(
                #     f"{maybe_nodes} entities, {maybe_edges} relations"
                # )
