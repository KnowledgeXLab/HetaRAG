...
def get_chunks(new_docs, chunk_func=chunking_by_token_size, **chunk_func_params):
    ...
    for chunk in chunks:
        inserting_chunks.update(
            {new_docs_list[chunk["chunk_order_index"]][1]["chunk_id"]: chunk}
        )
    # for i in range(len(chunks)):

    #     inserting_chunks.update(
    #                 {new_docs_list[i][1]["chunk_id"]: chunks[i]}
    #             )
...
async def _merge_edges_then_upsert(
    ...
    if src_id != " " and tgt_id != " ":
        ...
        order = min(
            [dp.get("order", 1) for dp in edges_data if len(dp) != 0] + already_order
        )
        weight = sum(
            [dp["weight"] for dp in edges_data if len(dp) != 0] + already_weights
        )
        description = GRAPH_FIELD_SEP.join(
            sorted(
                set(
                    [dp["description"] for dp in edges_data if len(dp) != 0]
                    + already_description
                )
            )
        )
        source_id = GRAPH_FIELD_SEP.join(
            set(
                [dp["source_id"] for dp in edges_data if len(dp) != 0]
                + already_source_ids
            )
        )
...
async def _find_most_related_text_unit_from_entities(
    ...
    all_text_units = [i for i in all_text_units if i["data"] != None]
...
async def _build_hierarchical_query_context(
    ...
    if use_communities:
        for c in use_communities:
            ...
            cur_community_key_entities.extend(  # 做交集加速运算，已经有了topk的，只需要取在社区里的就可以了
                [
                    e
...
def get_results(id):
    file_path = "/cpfs04/user/zhangyaoze/workspace/HiRAG/wto/tri.json"
    entity_results = []
    relation_results = []
    with open(file_path, "r") as f:
        for xline in f:
            ...
            e_dic[head_entity] = (
                dict(
                    entity_name=head_entity,
                    entity_type='"PERSON"',
                    description=relation,
                    source_id=id,
                ),
            )
            r_dic[(head_entity, tail_entity)] = (
                {},
                {
                    "src_tgt": head_entity,
                    "tgt_src": tail_entity,
