...
class HiRAG:
    ...
    def insert(self, string_or_strings, entity_results, relation_results):
        loop = always_get_an_event_loop()
        return loop.run_until_complete(
            self.ainsert(string_or_strings, entity_results, relation_results)
        )
    ...
    async def ainsert(self, string_or_strings, entity_results, relation_results):
        ...
        try:
            ...
            #     compute_mdhash_id(str(c['doc_name'])+"_"+str(c['page_idx'])+"_"+str(c["paragraph_idx"])): {"content": c["text"],
            #                                                                    "chunk_id":str(c['doc_name'])+"_"+str(c['page_idx'])+"_"+str(c["paragraph_idx"]),
            #                                                                    "doc_name":str(c['doc_name'])}
            #     for c in string_or_strings
            # }
            new_docs = {  # dict: {hash: ori_content}
                c["hash_code"]: {"content": c["text"], "chunk_id": str(i)}
                for i, c in enumerate(string_or_strings)
            }
