...
def _process_chunk(
    pdf_paths, pdf_backend, output_dir, num_threads, metadata_lookup, debug_data_path
):
        ...
        with ProcessPoolExecutor(max_workers=optimal_workers) as executor:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    processed_count += int(
                        result.split()[1]
                    )  # Extract number from "Processed X PDFs"
                    _log.info(
                    ...
                    if "prov" in table_item and table_item["prov"]:
                        ...
                        if page_num not in pages:
                            pages[page_num] = {
                                "page": page_num,
                                "content": [],
                                "page_dimensions": table_item["prov"][0].get(
                                    "bbox", {}
                                ),
                            }
                    ...
                    if "prov" in picture_item and picture_item["prov"]:
                        ...
                        if page_num not in pages:
                            pages[page_num] = {
                                "page": page_num,
                                "content": [],
                                "page_dimensions": picture_item["prov"][0].get(
                                    "bbox", {}
                                ),
                            }
...
class DoclingPDFParser:
    def __init__(self, pdf_file_name, output_dir: Path = "output"):
        self.pdf_file = Path(pdf_file_name)
        self.output_dir = Path(output_dir)
        parsed_reports_dirname = "01_parsed_reports"
        parsed_reports_debug_dirname = "01_parsed_reports_debug"
        merged_reports_dirname = f"02_merged_reports"
        reports_markdown_dirname = f"03_reports_markdown"
        self.parsed_reports_path = self.output_dir / parsed_reports_dirname
        self.parsed_reports_debug_path = self.output_dir / parsed_reports_debug_dirname
        self.merged_reports_path = self.output_dir / merged_reports_dirname
        self.reports_markdown_path = self.output_dir / reports_markdown_dirname

    def parse_pdf_report_sequential(self):
        logging.basicConfig(level=logging.DEBUG)

        pdf_parser = PDFParser(
            output_dir=self.parsed_reports_path,
        )
        pdf_parser.debug_data_path = self.parsed_reports_debug_path

        pdf_parser.parse_and_export(input_doc_paths=[self.pdf_file])
    ...
    def merge_reports(self):
        ...
        _ = ptp.process_reports(
            reports_dir=self.parsed_reports_path, output_dir=self.merged_reports_path
        )
    ...
    def export_reports_to_markdown(self):
        ...
        ptp.export_to_markdown(
            reports_dir=self.parsed_reports_path, output_dir=self.reports_markdown_path
        )
        print(f"Reports saved to {self.reports_markdown_path}")

    def process_pdf(self):
        print("Starting reports docling parser...")

        print("Step 1: Docling parser...")
        self.parse_pdf_report_sequential()

        print("Step 2: Merging reports...")
        self.merge_reports()

        print("Step 3: Exporting reports to markdown...")
