...
def load_submissions(root_path) -> List[AnswerSubmission]:
    files = (root_path).glob("*.json")
    submissions = []
    for f in files:
        if not f.stem.endswith("_debug"):
...
def load_canonic_answers(root_path, answers_dir):
    file = root_path / "answers.json"
    ...
    for submission in load_submissions(answers_dir):
        ...
        for q, data in schemas.items():
            ...
            val_score = max(
                [compare(data.kind, a, predicted.value) for a in data.answers]
            )

            # convert answer refs to hash:page format
            predicted_refs = [
                r.pdf_sha1 + ":" + str(r.page_index) for r in predicted.references
            ]
    ...
    df.to_csv(root_path / "ranking.csv", index=False)
