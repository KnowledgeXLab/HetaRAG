...
def convert_data_type(gme_embedding):

    array = gme_embedding.numpy()
    first_item = array[0]
    embedding = np.float64(first_item)

    return embedding


def get_embedding_Qwen2VL(gme, type, question):

    if type == "text":
        texts = [question]
        e_text = gme.get_text_embeddings(texts=texts)
        e_text = convert_data_type(e_text)
        return e_text
    elif type == "image":
        images = [question]
        e_image = gme.get_image_embeddings(images=images)
        e_image = convert_data_type(e_image)
        return e_image
    else:
...
# def get_embedding_ollama(question: str) -> List:
#     client = Client(host=f"http://{host}:{port}")
#     response = client.embeddings(model='bge-m3', prompt=question)
#     embedding = response['embedding']

#     return  np.float64(embedding)

from src.config.db_config import get_ollama_embedding

ollama_embedding_config = get_ollama_embedding()


def get_embedding_ollama(question):
    client = Client(
        host=f"http://{ollama_embedding_config['host']}:{ollama_embedding_config['port']}"
    )
    response = client.embeddings(model="bge-m3", prompt=question)
    embedding = response["embedding"]
    return np.float64(embedding)
...
def get_embedding_text(question):
...
if __name__ == "__main__":
    ...
    images = [
        "/data/maosong/tables/table_101_924c1df14763fe06a8d6efb16d2506f78c86878b8ac42834392d5b93a8dd8d68.jpg"
    ]

    # images = [
    #     '/data/maosong/images/image_1130_d28e2d23fc56deaf0debe0fc8a13a45b54f92b418602afb4591c475bcc558984.jpg'
    # ]
    # type = "image"
    ...
    if type == "image":
        collection_name = "deepwriter_image"
    else:
        collection_name = "deepwriter_text"
    host = "10.6.8.115"
    port = "19530"
    username = ""
    password = ""
